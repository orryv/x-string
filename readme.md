...
| [`between`](docs/x-string/methods/between.md) | 1.0 | `public function between(Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $start, Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $end, $last_occurence = false, int $skip_start = 0, int $skip_end = 0, string $start_behavior = 'sequential', string $end_behavior = 'sequential'): self`<br>Get the substring between two delimiters with optional reverse search and skip counters. Use `$start_behavior`/`$end_behavior` to decide whether arrays are matched sequentially or as OR alternatives. |
| [`before`](docs/x-string/methods/before.md) | 1.0 | `public function before(Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $search, $last_occurence = false, int $skip = 0, string $start_behavior = 'sequential'): self`<br>Get the substring before a specific delimiter. Control whether array inputs are treated as ordered fragments or OR alternatives via `$start_behavior`. |
| [`after`](docs/x-string/methods/after.md) | 1.0 | `public function after(Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $search, $last_occurence = false, int $skip = 0, string $start_behavior = 'sequential'): self`<br>Get the substring after a specific delimiter. Control whether array inputs are treated as ordered fragments or OR alternatives via `$start_behavior`. |
...
| [`betweenAll`](docs/x-string/methods/betweenAll.md) | 1.0 | `public function betweenAll(Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $start, Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $end, $reversed = false, string $start_behavior = 'sequential', string $end_behavior = 'sequential'): array`<br>Collect every substring between two delimiters, optionally reversing the order. Use the behavior flags to match array inputs sequentially or treat them as OR alternatives. |
...
| [`indexOf`](docs/x-string/methods/indexOf.md) | 1.0 | `public function indexOf(Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $search, bool $reversed = false, int\|bool $limit = 1, string $behavior = 'or'): false\|int\|array<int, int>`<br>Locate one or more occurrences of a candidate. `$limit = 1` returns a single index, `0`/`false` returns every match as an ordered array, integers `> 1` return up to that many matches in search order, and `$behavior` toggles between OR-style matching and sequential fragment matching (reporting the index of the last fragment in the sequence). |
