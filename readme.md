...
| [`betweenAll`](docs/x-string/methods/betweenAll.md) | 1.0 | `public function betweenAll(Newline\|HtmlTag\|Regex\|string\|array<Newline\|HtmlTag\|Regex\|string> $start, Newline\|HtmlTag\|Regex\|string $end, $reversed = false): array`<br>Get all substrings between two strings. If $reversed is true, it searches from the end of the string. Provide arrays to search for any matching delimiter; wrap sequences in nested arrays when fragments must appear consecutively. |
