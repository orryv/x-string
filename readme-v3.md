...
| [`toSnake`](docs/x-string/methods/toSnake.md) | 1.0 | `public function toSnake(Newline\|HtmlTag\|Regex\|string $input_delimiter = ' '): self`<br>Convert the string to snake_case. **The parameter specifies the input delimiter**; e.g. when `' '` is given, spaces are converted to `'_'`. Output always uses underscores. |
